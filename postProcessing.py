...
# except:
#     #compilation of cythonFunction
#     cmd = "python3 cythonSetup.py build_ext --inplace"
...
#         import cythonFunctions
#     except:
#         print("cythonFunctions not available, reverting to native python implementation")

# import sys
# cythonAvailable= "cythonFunctions" in sys.modules
...
def print3(matrix3):
    ...
    myStr='['+precString*3+"],"
    for i in range(3):
        print(myStr.format(*matrix3[i]))
    print()

def compactifySubVolume(V_fiberMap,fiberStruct,parallelHandle=True,returnAspectRatio=False):
    fiberDiameter=7.5 

    listMarkers=[val for val in np.unique(V_fiberMap) if val>=0]# tracked fibers have markers starting at 0

    printThetaPhi=len(listMarkers)<200 # this is mainly useful in debugging, with few fibers. Otherwise, it's cumbersome
        

    V_fiberMap_compactified=V_fiberMap.copy()
    
    currentMarkerVal=0

    markerLUT={}
    LUT_markerToQuaternion={}
    ...
    for iMark in listMarkers:
        
        fib=fiberStruct["fiberStruct"][iMark]

        fib.length=np.linalg.norm(fib.endPnt-fib.startPnt)
        if fib.rejected:
            raise ValueError("fiber is rejected, should not be in final fiberStruct")

        fib.orientationVecNormalized=fib.orientationVec/fib.length

        # This is because OpenFiberSeg doesn't use a right handed referential
        fib.orientationVecNormalized=np.array(
                [
                    fib.orientationVecNormalized[1],
                    fib.orientationVecNormalized[0],
                    fib.orientationVecNormalized[2]
                ]
            )


        fib.theta=np.arccos(np.dot(fib.orientationVecNormalized,np.array([0.,0.,1.])))

        fib.phi  =np.arctan2(fib.orientationVecNormalized[1],fib.orientationVecNormalized[0])
        

        thetaDeg=round(np.degrees(fib.theta))
        phiDeg  =round(np.degrees(fib.phi))

        if printThetaPhi:
            print("thetaDeg= {: >8.3f}\tphiDeg= {: >8.3f}".format(thetaDeg,phiDeg))
        
        if returnAspectRatio:
            AR_int=max(round(fib.length/fiberDiameter),1) #can't have a AR of 0, will crash later processing
            hashTuple=(thetaDeg,phiDeg,AR_int)
        else:
            hashTuple=(thetaDeg,phiDeg)
        

        if hashTuple not in dictThetaPhi_deg.keys():

            currentMarkerVal+=1
            markerLUT[iMark]=currentMarkerVal

            dictThetaPhi_deg[hashTuple]=\
                currentMarkerVal
            ...
            if returnAspectRatio:
                thetaDeg=-thetaDeg
            P=vs.eul2dcm( [-phiDeg, -thetaDeg,phiDeg], 'zyz', unit='deg')


            LUT_markerToQuaternion[currentMarkerVal]=\
                qt.from_rotation_matrix(P)
                
            ############################################    


            if returnAspectRatio:
                AR_LUT[currentMarkerVal]=AR_int
        else:
            markerLUT[iMark]=dictThetaPhi_deg[hashTuple]


    nZones=currentMarkerVal

    countPerMarker={}

    for v in markerLUT.values():
        countPerMarker[v]=len(np.where(np.array(list(markerLUT.values()))==v )[0])
    ...
    for iSlice,resTuple in enumerate(results):
        V_fiberMap_compactified[iSlice]=resTuple
    ...
    if returnAspectRatio:
        return V_fiberMap_compactified,LUT_markerToQuaternion,nZones,AR_LUT,printThetaPhi
    else:
        return V_fiberMap_compactified,LUT_markerToQuaternion,nZones,printThetaPhi
...
def postProcessingOfFibers(
    ...
    if randomize:
        ...
        times_postProc["random shuffling (CYTHON={}) in: ".format(cythonAvailable)]=time.strftime("%Hh%Mm%Ss", time.gmtime(tocRandomize-ticRandomize))
